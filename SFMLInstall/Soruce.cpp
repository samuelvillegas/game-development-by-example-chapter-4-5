...
void init() {
	...
	headingText.setString("Wizarditos");
	headingText.setCharacterSize(84);
	headingText.setFillColor(sf::Color::Magenta);
	...
	hero.init("Assets/graphics/heroAnim.png", 11, 1.2f, sf::Vector2f(viewSize.x * 0.25f, viewSize.y * 0.5f), 200);
...
void updateInput() {
	...
	while (window.pollEvent(event)) {
		if (event.type == sf::Event::KeyPressed) {
			...
			if (event.key.code == sf::Keyboard::Down || event.key.code == sf::Keyboard::Space) {
				if (gameover) {
					gameover = false;
					reset();
				}
